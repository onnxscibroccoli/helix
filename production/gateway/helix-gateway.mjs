...
async function desktop(req,res,id){const s=await verify(parseCookies(req)[COOKIE]);if(!s){res.writeHead(302,{location:"/auth/login"});return res.end()}if(!/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(id))return json(res,400,{error:"invalid workspace id"});const page='<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Helix Desktop</title><style>html,body,#screen{width:100%;height:100%;margin:0;background:#111}#status{position:fixed;z-index:2;top:8px;left:8px;color:#fff;font:14px system-ui;background:#222b;padding:6px 10px;border-radius:8px}#back{position:fixed;z-index:2;top:8px;right:8px;color:#fff;background:#222b;padding:6px 10px;border-radius:8px;text-decoration:none}</style><div id="status">Starting desktop…</div><a id="back" href="/">Workspaces</a><div id="screen"></div><script type="module">import RFB from "/novnc/core/rfb.js";const id=ID_PLACEHOLDER;const status=document.getElementById("status");async function connect(){const r=await fetch("/api/v1/workspaces/"+encodeURIComponent(id),{cache:"no-store"});if(!r.ok){status.textContent="Workspace unavailable";return}const x=await r.json();if(x.status!=="running"){status.textContent="Starting workspace…";setTimeout(connect,1500);return}const scheme=location.protocol==="https:"?"wss":"ws";const rfb=new RFB(document.getElementById("screen"),scheme+"://"+location.host+x.streamPath+"?ticket="+encodeURIComponent(x.ticket));rfb.scaleViewport=true;rfb.resizeSession=false;rfb.addEventListener("connect",()=>status.textContent="Connected");rfb.addEventListener("disconnect",()=>status.textContent="Disconnected");rfb.addEventListener("securityfailure",()=>status.textContent="Desktop security handshake failed");}connect();</script>';return html(res,200,page.replace("ID_PLACEHOLDER",JSON.stringify(id)))}
async function workspace(req,res,id){const s=await verify(parseCookies(req)[COOKIE]);if(!s){res.writeHead(401);return res.end("unauthorized")}if(!/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(id))return json(res,400,{error:"invalid workspace id"});let d=await hv("/domains/"+encodeURIComponent(id));if(d.status!==200)return json(res,d.status,{error:"workspace not found"});if(!d.body.owner || d.body.owner!==s.sub)return json(res,403,{error:"workspace not authorized"});if(d.body.status!=="running"){const started=await hv("/domains",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({id,kind:d.body.kind==="ephemeral"?"ephemeral":"persistent",owner:s.sub})});if(started.status>=400)return json(res,started.status,{error:"workspace failed to start"});d=started;}if(d.body.status!=="running"||d.body.display==null)return json(res,409,{error:"workspace is not ready yet"});const ticket=await sign({sub:s.sub,workspace:id,kind:"desktop"},WS_TTL);return json(res,200,{workspace:id,status:d.body.status,display:d.body.display,streamPath:"/kasm/ws/"+id,ticket})}
